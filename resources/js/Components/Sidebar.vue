...
const props = defineProps({
    ...
    // BARU: State dari StafLayout buat ngebaca mobile buka/tutup
    isMobileOpen: {
        type: Boolean,
        default: false
    }
...
// Helper: pastikan di HP sidebar selalu tampil lebar (text terlihat)
const isMiniMode = computed(() => props.isCollapsed && !props.isMobileOpen);
...
<template>
    <!-- OVERLAY GELAP UNTUK MOBILE (Klik di luar sidebar buat tutup) -->
    <div v-if="isMobileOpen"
         @click="emit('closeMobile')"
         class="fixed inset-0 z-40 transition-opacity bg-black/50 lg:hidden backdrop-blur-sm">
    </div>

    <aside
        class="fixed top-0 left-0 z-50 flex flex-col h-screen transition-all duration-300 border-r bg-base-100 border-base-300"
        :class="[
            isMobileOpen ? 'translate-x-0 w-64' : '-translate-x-full lg:translate-x-0', // Default tertutup di mobile
            isMiniMode ? 'lg:w-20' : 'lg:w-64'
        ]"
    >
        <div class="relative flex items-center justify-center h-16 border-b shrink-0 border-base-300" :class="isMiniMode ? 'px-2' : 'px-6'">
            <span v-if="!isMiniMode" class="text-xl italic font-black tracking-tighter text-primary drop-shadow-sm">
                BIKIN<span class="text-base-content">CETAK</span>
            </span>
            <span v-else class="hidden text-2xl italic font-black tracking-tighter text-primary drop-shadow-sm lg:block">
                B<span class="text-base-content">C</span>
            </span>

            <!-- TOMBOL CLOSE KHUSUS MOBILE -->
            <button v-if="isMobileOpen" @click="emit('closeMobile')" class="absolute p-2 lg:hidden right-3 btn btn-ghost btn-sm btn-circle hover:bg-base-200">
                <svg class="w-5 h-5 text-base-content/70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        ...
        <nav class="flex-1 py-6 space-y-2 overflow-y-auto pb-24 lg:pb-6" :class="isMiniMode ? 'px-2' : 'px-4'">
            ...
            <Link :href="route('dashboard')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('dashboard'),
                    'hover:translate-x-1': !isMiniMode && !route().current('dashboard'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Dashboard' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Dashboard</span>
            </Link>
            ...
            <Link v-if="$can('akun')" :href="route('akun.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('akun.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('akun.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Data Akun COA' : ''"
                @click="emit('closeMobile')"
            >
                <svg class="w-5 h-5 shrink-0" :class="!isMiniMode && 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"></path></svg>
                <span v-if="!isMiniMode">Data Akun COA</span>
            </Link>
            ...
            <Link v-if="$can('customer')" :href="route('customer.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('customer.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('customer.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Data Customer' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Data Customer</span>
            </Link>
            ...
            <Link v-if="$can('staf')" :href="route('staf.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('staf.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('staf.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Data Staf' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Data Staf</span>
            </Link>
            ...
            <Link v-if="$can('vendor')" :href="route('vendor.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('vendor.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('vendor.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Data Vendor' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Data Vendor</span>
            </Link>
            ...
            <div v-if="$can('kategori') || $can('produk') || $can('varian') || $can('produk-sku') || $can('finishing') || $can('voucher')" class="space-y-1">
                <button @click="toggleProductDropdown"
                    ...
                    :class="{
                        ...
                        'translate-x-1': !isMiniMode && (route().current('produk.*') || route().current('finishing.*') || route().current('kategori.*') || route().current('varian.*') || route().current('voucher.*') || route().current('sku.*')),
                        'hover:translate-x-1': !isMiniMode && !(route().current('produk.*') || route().current('finishing.*') || route().current('kategori.*') || route().current('varian.*') || route().current('voucher.*') || route().current('sku.*')),
                        'justify-center px-0': isMiniMode,
                        'justify-between px-4': !isMiniMode
                    }"
                    :title="isMiniMode ? 'Data Produk' : ''"
                >
                    <div class="flex items-center" :class="isMiniMode && 'justify-center w-full'">
                        <svg class="w-5 h-5 shrink-0" :class="!isMiniMode && 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                        <span v-if="!isMiniMode">Data Produk</span>
                    </div>
                    <svg v-if="!isMiniMode" class="w-4 h-4 transition-transform duration-300 shrink-0"
                ...
                <div v-show="isProductDropdownOpen && !isMiniMode" class="pl-2 mt-1 ml-4 space-y-2 border-l-2 border-base-300/50">
            ...
            <Link v-if="$can('bahan-baku')" :href="route('bahan-baku.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('bahan-baku.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('bahan-baku.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Bahan Baku' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Bahan Baku</span>
            </Link>
            ...
            <Link v-if="$can('pembelian-bahan')" :href="route('pembelian-bahan.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('pembelian-bahan.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('pembelian-bahan.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Pembelian Bahan' : ''"
                @click="emit('closeMobile')"
            >
            ...
            <Link v-if="$can('pesan')" :href="route('pesan.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('pesan.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('pesan.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Pesanan' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Pesanan</span>
            </Link>
            ...
            <Link v-if="$can('produksi')" :href="route('produksi.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('produksi.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('produksi.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Produksi' : ''"
                @click="emit('closeMobile')"
            >
                <svg class="w-5 h-5 shrink-0" :class="!isMiniMode && 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span v-if="!isMiniMode">Produksi</span>
            </Link>
            ...
            <Link v-if="$can('pembayaran')" :href="route('pembayaran.index')"
                ...
                :class="{
                    ...
                    'translate-x-1': !isMiniMode && route().current('pembayaran.*'),
                    'hover:translate-x-1': !isMiniMode && !route().current('pembayaran.*'),
                    'justify-center px-0': isMiniMode,
                    'px-4': !isMiniMode
                }"
                :title="isMiniMode ? 'Pembayaran' : ''"
                @click="emit('closeMobile')"
            >
                ...
                <span v-if="!isMiniMode">Pembayaran</span>
            </Link>
            ...
            <div v-if="$can('hak-akses') || $can('file-manage')" class="space-y-1">
                <button @click="toggleSettingsDropdown"
                    ...
                    :class="{
                        ...
                        'translate-x-1': !isMiniMode && (route().current('hak-akses.*') || route().current('file-manage.*')),
                        'hover:translate-x-1': !isMiniMode && !(route().current('hak-akses.*') || route().current('file-manage.*')),
                        'justify-center px-0': isMiniMode,
                        'justify-between px-4': !isMiniMode
                    }"
                    :title="isMiniMode ? 'Pengaturan' : ''"
                >
                    <div class="flex items-center" :class="isMiniMode && 'justify-center w-full'">
                        <svg class="w-5 h-5 shrink-0" :class="!isMiniMode && 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path></svg>
                        <span v-if="!isMiniMode">Pengaturan</span>
                    </div>
                    <svg v-if="!isMiniMode" class="w-4 h-4 transition-transform duration-300 shrink-0"
        ...
        <div class="hidden p-4 border-t lg:block border-base-300 bg-base-100 shrink-0" :class="isMiniMode ? 'flex justify-center' : ''">
            <div class="flex items-center px-3 py-3 border bg-base-200 rounded-2xl border-base-300/50" :class="isMiniMode ? 'justify-center p-2' : ''">
                <div class="avatar placeholder" :class="!isMiniMode && 'mr-3'">
                    <div class="flex items-center justify-center font-black text-white rounded-lg shadow-sm bg-primary w-9 h-9">
                        {{ $page.props.auth.user.name.charAt(0).toUpperCase() }}
                    </div>
                </div>
                <div v-if="!isMiniMode" class="flex-1 min-w-0">
