...
<template>
    ...
    />
    <div>
        <!-- Header Terpisah (Tanpa Card Pembungkus) -->
        <div class="flex items-center justify-between mb-2 px-3 py-3 rounded-xl bg-base-100">
            ...
            <CustomButton @click="$emit('addItem')" size="sm">+ Tambah Item</CustomButton>
        </div>

        <!-- Tabel Murni tanpa Spasi Bawah -->
        <div class="w-full">
                ...
                <tr v-for="(item, index) in items" :key="item.id || item.cart_id || index" class="transition-colors border-b hover:bg-base-200/30 border-base-200/50">
                    ...
                    <td class="pt-3 text-right align-top">
                        <div class="flex flex-col items-end gap-1">
                            ...
                            <!-- Badge Diskon Customer -->
                            <div v-if="item.rincian_diskon_snapshot?.length" class="flex flex-col items-end gap-0.5 mt-0.5">
                                <span v-for="(diskon, dIdx) in item.rincian_diskon_snapshot" :key="'dsc'+dIdx" class="text-[8px] font-black uppercase tracking-wider text-error opacity-90 bg-error/10 px-1.5 py-0.5 rounded">
                                    {{ diskon.nama }}
                                </span>
                            </div>
                        </div>
                    ...
                    <td class="pt-3 text-right align-top">
                        <div class="flex flex-col items-end gap-1">
                            ...
                            <div v-if="(item.harga_pengerjaan_snapshot || 0) > 0" class="mt-0.5">
                                <span class="text-[8px] font-black uppercase tracking-wider text-warning opacity-90 bg-warning/10 px-1.5 py-0.5 rounded">
                    ...
                    <!-- KOLOM 6: AKSI (CustomTableAction) -->
                    <td class="pt-3 text-center align-top">
                        <CustomTableAction v-slot="{ close }">
                            ...
                            <button @click="$emit('requestEdit', item); close()" class="flex items-center w-full px-4 py-2.5 text-xs font-bold text-base-content hover:bg-base-200 transition-colors">
                            ...
                            <button @click="openDeleteModal(item); close()" class="flex items-center w-full px-4 py-2.5 text-xs font-bold text-error hover:bg-error/10 transition-colors">
