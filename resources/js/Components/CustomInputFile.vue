...
watch(() => props.modelValue, (newVal) => {
    if (newVal) {
        activeTab.value = newVal.tipe_file || 'upload';
...
<template>
    <div class="w-full max-w-full min-w-0 p-4 space-y-3 overflow-hidden transition-colors border bg-base-100 rounded-2xl" :class="error ? 'border-error/50 bg-error/5' : 'border-base-content/10'">
        ...
        <div class="grid w-full grid-cols-3 p-1 rounded-lg join bg-base-200">
            <button type="button" @click="setActiveTab('upload')" :class="['join-item btn btn-xs border-none font-black text-[9px] uppercase', activeTab === 'upload' ? 'bg-base-100 shadow-sm text-primary' : 'bg-transparent text-base-content/50 hover:bg-base-300']">Upload</button>
            <button type="button" @click="setActiveTab('link')" :class="['join-item btn btn-xs border-none font-black text-[9px] uppercase', activeTab === 'link' ? 'bg-base-100 shadow-sm text-primary' : 'bg-transparent text-base-content/50 hover:bg-base-300']">Link</button>
            <button type="button" @click="setActiveTab('email')" :class="['join-item btn btn-xs border-none font-black text-[9px] uppercase', activeTab === 'email' ? 'bg-base-100 shadow-sm text-primary' : 'bg-transparent text-base-content/50 hover:bg-base-300']">Email</button>
        </div>

        <div v-if="activeTab === 'upload'" class="mt-3">
            <input type="file" class="hidden" ref="fileInputRef" @change="handleFileChange" accept=".pdf,.ai,.jpg,.jpeg,.png,.zip" />
            <button v-if="!selectedFile" type="button" @click="fileInputRef.click()" :class="['btn btn-outline border-dashed border-2 w-full flex items-center justify-start gap-3 rounded-xl h-11 bg-base-200/30 hover:bg-base-200 min-w-0 px-4 overflow-hidden', internalError ? 'border-error bg-error/5 text-error' : 'border-base-300']">
            ...
            <div v-else class="flex items-center w-full min-w-0 gap-3 p-2 border bg-primary/10 rounded-xl border-primary/20 h-11">
                <div class="bg-primary text-white p-1.5 rounded-lg shrink-0">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-3 h-3"><path d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"/><polyline points="13 2 13 9 20 9"/></svg>
                </div>
                ...
                <button type="button" @click="removeFile" class="mr-1 btn btn-ghost btn-circle btn-xs text-error shrink-0">
        ...
        <div v-if="activeTab === 'link'" class="mt-3">
            <input type="url" placeholder="https://drive.google.com/..." :value="linkDrive" @input="updateLink" class="w-full text-xs font-bold transition-all input input-bordered h-11 rounded-xl bg-base-100 focus:border-primary focus:ring-2 focus:ring-primary/20 border-base-300" />
        </div>
