...
const isEdit = !!props.staf;

const form = useForm({
    name: props.staf?.user?.name ?? '',
    email: props.staf?.user?.email ?? '',
    password: '',
    no_hp: props.staf?.no_hp ?? '',
    id_role_staf: props.staf?.id_role_staf ?? '',
});

const addNewRole = (val) => {
    const roleName = prompt("Masukkan nama role baru:", val);
    if (roleName) {
        router.post(route('role-staf.store'), { role: roleName }, { preserveScroll: true });
    }
};

const submit = () => {
    if (isEdit) {
        form.put(route('staf.update', props.staf.id_staf));
    } else {
        form.post(route('staf.store'));
    }
};
</script>

<template>
    <Head :title="isEdit ? 'Edit Staf' : 'Registrasi Staf'" />
                    ...
                    <form @submit.prevent="submit" class="space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <CustomSelectSearch
                                    v-model="form.id_role_staf"
                                    label="Level Member"
                                    :options="roles"
                                    labelKey="role"
                                    valueKey="id_role_staf"
                                    :error="form.errors.id_role_staf"
                                    @onCreate="addNewRole"
                                />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-center gap-4 pt-6 sm:flex-row">
                            ...
                            >
                                ...
                                {{ isEdit ? 'Perbarui Data Staf' : 'Konfirmasi & Simpan Staf' }}
                            </CustomButton>
