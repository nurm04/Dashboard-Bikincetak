...
<template>
    ...
    <StafLayout>
        ...
        <div class="min-h-screen px-4 py-6 mx-auto sm:px-6 lg:px-8 max-w-7xl">
            ...
            <!-- BAGIAN ATAS: TOMBOL & FILTER DIRAPIKAN -->
            <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">

                <!-- Wrapper Tombol Tambah pakai props "block" yang baru dibuat -->
                <div class="w-full md:w-auto">
                    <CustomButton v-if="$can('produk', 'tambah')" type="link" :href="route('produk.create')" variant="primary" block>
                        <template #icon>
                ...
                <tr v-for="prd in produks" :key="prd.id_produk" class="transition-colors hover:bg-base-200/50">
                    ...
                    <td class="px-4 py-4 min-w-50">
                        <div class="flex flex-wrap gap-1">
                            <span v-for="v in prd.varians" :key="v.id_varian" class="px-2 py-0.5 rounded-md bg-success/10 text-[10px] font-bold uppercase tracking-tight text-success border border-success/20">
                                {{ v.nama_varian }}
                            </span>
                            <span v-if="!prd.varians.length" class="text-[10px] italic opacity-40">Belum ada varian</span>
                        </div>
                    </td>

                    <td class="px-4 py-4 text-sm font-bold whitespace-nowrap">{{ prd.produk_sku?.length || 0 }} SKU</td>
